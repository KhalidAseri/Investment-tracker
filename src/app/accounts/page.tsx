        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {accounts.map((account) => {
            ...
            const totalValue = holdings.reduce(
              (sum, h) => sum + (h.currentValue ?? h.shares * h.averageCost),
              0
            )
                ...
                <Link href={`/accounts/${account.id}`} className="block">
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-4">
                    <div>
                      ...
                      <CurrencyDisplay amount={totalValue} className="text-sm font-semibold" />
                    </div>
                    <div>
                      <p className="text-xs text-gray-500">{t.accounts.cost}</p>
                      <CurrencyDisplay amount={totalCost} className="text-sm font-semibold" />
                    </div>
                  </div>
                  ...
                  <p className="text-xs text-gray-400 mt-3">
                    {account._count?.holdings ?? holdings.length} {t.accounts.holdings} &middot;{' '}
                    {account._count?.transactions ?? 0} {t.accounts.transactions}
                  </p>
                </Link>
